...
export function AccountsManagerClean() {
  const { accounts, loading, refreshAccounts, refreshData } = useFinance()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingAccount) {
        await cleanApiClient.put<Account>("accounts", editingAccount.id, accountData)
      } else {
        await cleanApiClient.post<Account>("accounts", accountData)
      }

      // Atualizar todos os dados após a modificação
      await refreshData()
      resetForm()
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await cleanApiClient.delete("accounts", id)
      await refreshData()
    } catch (error) {
